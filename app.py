...
FACE_FEATURES_CACHE = os.path.join(BASE_DIR, 'data', 'face_features.json')

# 导入 Dlib 人脸识别模块（替换旧的 Haar+直方图方案）
try:
...
app.config['PERMANENT_SESSION_LIFETIME'] = 86400 * 30  # 30 天
CORS(app)
...
def api_login():
    ...
    if user and _verify_password(password, user.get('salt', ''), user.get('password_hash', '')):
        ...
        if remember:
            session.permanent = True  # 30 天有效期
...
def api_system_ip():
    ...
    try:
        ...
        ip = '未知'
    return jsonify({'ok': True, 'ip': ip, 'port': 5000})
...
def _build_system_prompt():
    ...
    try:
        ...
        if os.path.isfile(att_file):
            ...
            if att_records:
                recent = list(att_records.values())[-5:]
                total_att = len(att_records)
                today = datetime.utcnow().strftime('%Y-%m-%d')
                today_key = f'recording_{today}'
                today_count = 0
                for k, v in att_records.items():
                    if today in k:
                        today_count = len(v.get('records', [])) if isinstance(v, dict) else 0
                        break
                parts.append(f'累计考勤记录{total_att}条。今天({today})考勤了{today_count}人。')
    except:
        pass
    return '\n'.join(parts)
...
# ============================================================================
# YOLO 模型初始化（项目2，可选）—— 使用 class_Detection_end 的 6 类行为检测模型
# ============================================================================
...
try:
    ...
    print("提示: YOLO 依赖未安装 (ultralytics)，行为检测功能不可用，人脸考勤正常")

if _yolo_available and os.path.isfile(YOLO_MODEL_PATH):
    try:
        yolo_model = YOLO(YOLO_MODEL_PATH)
        TRACKED_CLASSES = tuple([str(v) for k, v in sorted(yolo_model.names.items(), key=lambda x: int(x[0]))])
        print(f"YOLO 模型已加载: {YOLO_MODEL_PATH} ({DEVICE}) — 6类行为检测")
    except Exception as e:
        print(f"YOLO 模型加载失败: {e}")
else:
    if not _yolo_available:
        print("YOLO 未启用: 缺少 ultralytics 依赖")
...
# OpenCV 摄像头（项目1）
# ============================================================================
_cameras = {}            # camera_id -> cv2.VideoCapture
...
def release_camera(camera_id=0):
    """释放指定摄像头，不传 id 则释放全部"""
    global _cameras
    with _cameras_lock:
...
def match_student(image_path):
    """
    使用 Dlib ResNet CNN 进行人脸匹配 —— 替代原来的 Haar + 直方图方案。
    接口与旧版完全兼容。
    """
    return face_rec.match_face_dlib(
...
def recognize_action(image_path, use_onnx=True):
    try:
        ...
        if use_onnx and os.path.exists(os.path.join(BASE_DIR, 'model', 'cnn_model.onnx')):
            import onnxruntime as ort
            session = ort.InferenceSession(os.path.join(BASE_DIR, 'model', 'cnn_model.onnx'))
            result = session.run([session.get_outputs()[0].name], {session.get_inputs()[0].name: input_data})
            output = result[0][0]
            pred_idx = int(np.argmax(output))
        else:
            import paddle
            import cnn_model
            layer_state_dict = paddle.load(os.path.join(BASE_DIR, 'model', 'cnn_layer.pdparams'))
            model_cnn = cnn_model.CNN()
            model_cnn.set_state_dict(layer_state_dict)
            model_cnn.eval()
            output = model_cnn(paddle.to_tensor(input_data)).numpy()[0]
            pred_idx = int(np.argmax(output))

        action_labels = ['听讲', '举手', '低头', '站立', '走动']
        from scipy.special import softmax
        prob = softmax(output)
        return {'action': action_labels[pred_idx], 'confidence': f'{prob[pred_idx]*100:.1f}%'}
    except Exception as e:
...
def api_add_student():
    ...
    # 重建人脸特征数据库
    face_rec.rebuild_features_on_register(UPLOAD_FOLDER, STUDENTS_INFO_FILE, FACE_FEATURES_CACHE)
...
def api_update_student():
    ...
    if not person_id:
        return jsonify({'ok': False, 'error': 'no person_id'}), 400
    old_info = {}
    if os.path.exists(STUDENTS_INFO_FILE):
        with open(STUDENTS_INFO_FILE, 'r', encoding='utf-8') as f:
            old_info = json.load(f).get('students', {}).get(person_id, {})
    old_sid = old_info.get('student_id', '')
    new_name = name if name else old_info.get('name', '')
    new_class = class_name if class_name else old_info.get('class', '')
    new_sid = new_student_id if new_student_id else old_sid
    if new_sid and new_sid != old_sid:
        new_pid = f'person_{new_sid}'
        old_folder = os.path.join(UPLOAD_FOLDER, person_id)
        new_folder = os.path.join(UPLOAD_FOLDER, new_pid)
        if os.path.exists(old_folder) and not os.path.exists(new_folder):
            os.rename(old_folder, new_folder)
        if os.path.exists(FACE_FEATURES_CACHE):
            with open(FACE_FEATURES_CACHE, 'r', encoding='utf-8') as f:
                feat = json.load(f)
            if person_id in feat:
                feat[new_pid] = feat.pop(person_id)
                with open(FACE_FEATURES_CACHE, 'w', encoding='utf-8') as f:
        ...
        if os.path.exists(STUDENTS_INFO_FILE):
            ...
            if person_id in data.get('students', {}):
                ...
                with open(STUDENTS_INFO_FILE, 'w', encoding='utf-8') as f:
                    json.dump(data, f, ensure_ascii=False, indent=2)
        update_student_info(new_pid, new_name, new_sid, new_class)
    else:
        update_student_info(person_id, new_name, new_sid, new_class)
    return jsonify({'ok': True, 'message': f'ok: {new_name}'})
    ...
    try:
        ...
        while True:
            ...
            if frame_idx % frame_interval == 0:
                processed_frames += 1
                # 转换为RGB并保存临时帧（cv2.imwrite 会将数据原样写入，
                # 所以必须先把 BGR 转为 RGB，否则后续 PIL 读取时红蓝通道颠倒）
...
def api_face_records():
    ...
    if os.path.exists(FACE_ATTENDANCE_FILE):
        ...
        records = [r for r in all_records if r.get('timestamp', '').startswith(today)]
...
def api_cnn_predict():
    ...
    import subprocess
    try:
        result = subprocess.run([sys.executable, 'main.py'], capture_output=True, text=True, cwd=BASE_DIR)
...
def api_yolo_predict():
    ...
    if media_type == 'image':
        # 使用 class_Detection_end 的简化推理方式
        import cv2
...
def api_yolo_predict_frame():
    ...
    # 直接使用 YOLO 模型推理（BGR 格式）
    arr = np.asarray(img)[:, :, ::-1].copy()
    result = yolo_model.predict(arr, conf=0.15, verbose=False)[0]
    stats = _stats_from_result(result)
    ...
    return jsonify({'image_base64': base64.b64encode(buffer.getvalue()).decode('utf-8'), 'stats': stats, 'boxes': boxes_out})
...
def api_yolo_capture():
    ...
    try:
        ...
        return jsonify({'ok': True, 'original': f'/api/yolo/captures/originals/{orig_name}', 'annotated': f'/api/yolo/captures/annotated/{annot_name}', 'stats': stats, 'stats_url': f'/api/yolo/captures/stats/{stats_name}'})
    except Exception as e:
...
def generate_frames_with_detect(camera_id):
    """生成带YOLO检测框的视频帧"""
    import base64
    ...
    while True:
        with _cameras_lock:
            if camera_id not in _cameras:
                break
            success, frame = _cameras[camera_id].read()
        if not success:
            break
        ...
        if yolo_model is not None:
            try:
                result = yolo_model.predict(frame, conf=0.15, verbose=False)[0]
                boxes = result.boxes
                if boxes is not None:
                    # 转换为PIL绘制中文
                    ...
                    for box in boxes:
                        ...
                        cls_name_en = result.names.get(cls_id, str(cls_id))
...
def video_feed_detect():
    ...
    camera = get_camera(camera_id)
    if camera is None:
        return "摄像头不可用 (编号: {})".format(camera_id), 503
    return Response(generate_frames_with_detect(camera_id),
...
def api_energy_status():
    return jsonify({
        ...
        'simulate': True, 'model_loaded': False,
        'fps': 0, 'message': 'PC 端不支持节能控制，请使用 RK3588 开发板'
    })

@app.route('/api/energy/override', methods=['POST'])
def api_energy_override():
    return jsonify({'ok': False, 'error': 'PC 端不支持节能控制'})
...
@app.route('/api/yolo/latest', methods=['GET'])
def api_yolo_latest():
    """获取最新的检测结果"""
    with detections_lock:
        return jsonify({
            ...
            'stats': latest_detections['stats']
        })

@app.route('/api/yolo/update', methods=['POST'])
def api_yolo_update():
    """更新检测结果（供 Qt 界面调用）"""
