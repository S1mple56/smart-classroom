...
# 使用模型进行分类
def predict(model, eval_reader, people_number, result_txt_path, student_info=None):
    """
    对于每个人的所有图片数据, 对 CNN 模型改进为：
        1. 判断该人的所有图片, 对该人文件夹内的所有图片进行类别划分, 并将对应的数字化类别存放于列表中
        2. 对列表进行处理, 列表中种类最多的类别便是当前人所对应的行为类别
        3. 新建文本文档, 将当前人的序号、姓名、学号、班级与行为类别一一对应存入.txt中
    """
    model.eval()

    with open(const.MODEL_INFO_PATH, 'r', encoding='utf-8') as f_model_info:
        model_info = json.load(f_model_info)
        label_dict = model_info["class_detail"]

    for batch_id, data in enumerate(eval_reader()):
        x_data = numpy.array(data, dtype='float32').reshape(-1, 3, 64, 64)
        ...
        student_id = student_info.get("student_id", "未知") if student_info else "未知"
        class_name = student_info.get("class", "未知") if student_info else "未知"
        
        # 计算置信度
        total = len(result_list)
        confidence = Counter(result_list)[result] / total * 100
        
        with open(result_txt_path, 'a', encoding='utf-8') as f_result:
            f_result.write(f"{people_number}\t{name}\t{student_id}\t{class_name}\t{label_dict[str(result)]['class_name']}\t{confidence:.1f}%\n")
