...
with open(const.EVAL_TEXT_PATH, 'w', encoding='utf-8') as f_eval:
    f_eval.seek(0)
    f_eval.truncate()
...
# 清空结果文件并写入表头
with open(result_txt_path, 'w', encoding='utf-8') as f_result:
    f_result.write("序号\t姓名\t学号\t班级\t行为\t置信度\n")
...
# 读取学生信息文件
try:
    with open(const.STUDENTS_INFO_PATH, 'r', encoding='utf-8') as f:
        students_info_data = json.load(f)
        students_info = students_info_data.get("students", {})
except FileNotFoundError:
    print(f"警告：未找到学生信息文件 {const.STUDENTS_INFO_PATH}")
    students_info = {}
...
for class_dir in class_dirs:
    if class_dir != '.DS.Store':
        ...
        # 当前人的照片输入网络模型进行预测
        cnn_model.predict(model, eval_reader, people_number, const.RESULT_TEXT_PATH, student_info)

print(f"预测完成！结果已保存到 {result_txt_path}")
